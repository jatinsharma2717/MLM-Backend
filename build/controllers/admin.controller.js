...
const url = "mongodb+srv://jatinsharmaaj:123456Jhu@cluster0.nxypk97.mongodb.net";
...
const updateUserStatus = async (req, res) => {
    ...
    try {
        ...
        if (req.body.status == constant_1.ACCOUNT_STATUS.ACTIVE) {
            ...
            const level1User = await user_1.Users.findOne({ userid: +referbyuserid });
            ...
            const level2User = await user_1.Users.findOne({
            ...
            if (level2User?.referbyuserid) {
                ...
                if (level2User?.referbyuserid) {
                    const level3User = await user_1.Users.findOne({
                    ...
                    if (level3User?.referbyuserid) {
                        ...
                        const level4User = await user_1.Users.findOne({
                        ...
                        if (level4User?.referbyuserid) {
                            ...
                            const level5User = await user_1.Users.findOne({
...
const getPaymentDetailsForAdmin = async (req, res) => {
    try {
        ...
        const user = await payment_1.paymentDetails.findOne({ userid: req.params.userid });
        const responseBody = {
            data: user,
            message: "success",
            statusCode: 200,
...
const getQrCodeDetailsForAdmin = async (req, res) => {
    try {
        ...
        const fileDetails = await payment_1.paymentDetails.findOne({
        ...
        const client = new mongodb_1.MongoClient("mongodb+srv://jatinsharmaaj:123456Jhu@cluster0.nxypk97.mongodb.net");
...
const payPaymentToUser = async (req, res) => {
    ...
    try {
        let user = await payment_to_user_1.PayemntToUser.findOne({ userid });
        if (!user) {
...
const getPaymentIncome = async (req, res) => {
    try {
        ...
        const existingUserLevel = await user_level_1.UserLevels.findOne({
            userid: req.params.userid,
        });
