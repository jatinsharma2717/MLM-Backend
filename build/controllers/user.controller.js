...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (referDetails) {
            ...
            const level1User = await user_2.Users.find({ userid: +referbyuserid });
            ...
            const level2User = await user_2.Users.find({
            ...
            if (level2User?.referbyuserid) {
                ...
                if (level2User?.referbyuserid) {
                    const level3User = await user_2.Users.find({
                    ...
                    if (level3User?.referbyuserid) {
                        ...
                        const level4User = await user_2.Users.find({
                        ...
                        if (level4User?.referbyuserid) {
                            ...
                            const level5User = await user_2.Users.find({
...
const loginUser = async (req, res) => {
    try {
        ...
        const existingUser = await user_2.Users.find({ userid: userid });
        if (!existingUser) {
            return res.status(400).send({
...
const getUserDetails = async (req, res) => {
    ...
    const encrypteduserId = await (0, security_service_1.userIdDecryption)(accountid.toString());
    try {
        const user = await user_2.Users.find({ userid: +encrypteduserId });
        const responseBody = {
            data: user,
            message: "success",
            statusCode: 200,
